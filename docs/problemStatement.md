# Weekly Product Review Pulse — Challenge Statement
...
**Initial products:** INDMoney, Groww, PowerUp Money, Wealth Monitor, and Kuvera.

## Challenge

Build an AI agent that ingests recent public reviews from the Apple App Store and Google Play, identifies recurring customer themes, selects representative real quotes, and generates a one-page weekly product review pulse.
...
- Collect public reviews from the last 8–12 weeks for each product.
- Clean, deduplicate, and scrub sensitive information from review text.
- Cluster reviews into meaningful themes using embeddings and/or similar NLP techniques.
- Use an LLM to generate:
  - top customer themes
  - representative quotes
  - actionable product or support ideas
  - a short stakeholder-friendly summary
- Validate that all quotes used in the report come from actual review text.
- Generate one concise weekly report per product.
- Deliver or export the report through Google Workspace, such as Google Docs and Gmail, or provide a clearly defined integration path.

## Key Requirements

- The orchestration must be built using **LangChain or LangGraph**.
...
- real-time analytics
- a BI dashboard
- social media sources such as Reddit or Twitter
- support for all fintech apps beyond the initial product list
...
1. **App performance and bugs** — Users report lag, crashes, and login issues.
2. **Customer support delays** — Users mention slow responses and unresolved tickets.
3. **UX and feature gaps** — Users ask for clearer navigation and better portfolio insights.
